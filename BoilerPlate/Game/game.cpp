...
namespace game
{
	...
	{
		reset_input_limiter();
		mBall.get_component("model_matrix")->get_model_matrix()->translate(engine::math::Vector4(0.3f, -0.4f, 0.0f, 0.0f));
	...
	void game::update_input()
	{
		...
		if (mInputManager.A_IsPressed() && mPaddle.get_component("position")->get_position().x > -1.45f)
		{
			mPaddle.translate(engine::math::Vector4(-0.025f, 0.0f, 0.0f, 0.0f));

		}

		if (mInputManager.D_IsPressed() && mPaddle.get_component("position")->get_position().x < 1.45f)
		{
			mPaddle.translate(engine::math::Vector4(0.025f, 0.0f, 0.0f, 0.0f));
		}
